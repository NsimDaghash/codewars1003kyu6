/*
Finite State Automata 1: Traffic Lights
...
class StateMachine {
  ...
  constructor({ init, transitions }) {
    ...
    // Create transition methods dynamically
    for (const transition of transitions) {
      this.createTransitionMethod(transition);
    }
  }

  createTransitionMethod(transition) {
    const { current, target, method } = transition;

    // Create a transition method for the given transition rule
    this[method] = () => {
      if (this.state === current) {
        this.state = target;
        console.log(`Transitioning from ${current} to ${target}`);
      } else {
        console.log(
          `Invalid transition: Cannot go from ${this.state} to ${target}`
        );
      }
    };
  }
