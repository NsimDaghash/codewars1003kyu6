...
const ticker = (text, width, tick) => {
  const emptySpace = " ".repeat(width);
  const tickIndex = tick % (text.length + width);
  let displayedText;

  if (tickIndex >= width) {
    displayedText = text.slice(tickIndex - width, tickIndex);
  } else {
    const padding = " ".repeat(width - tickIndex);
    displayedText = padding + text.slice(0, tickIndex);
  }

  return displayedText.padEnd(width, " ");
};
